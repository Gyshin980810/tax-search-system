 ...
 *   3. OpenAI text-embedding-3-small로 배치 임베딩 생성
...
if (!OPENAI_API_KEY) {
  console.error('[embed] OPENAI_API_KEY 환경변수가 필요합니다.')
  process.exit(1)
}
...
/**
 * 배치 크기 — OpenAI Embeddings API 단일 요청 최대 300,000 토큰.
 * 비법령(심판례·해석례) 본문은 최대 6,000자(약 6,000 토큰) × 20건 = 120,000 토큰으로 안전.
 * 법령(짧은 조문) 기준 100에서 20으로 축소 — TAX-053 비법령 적재 시 토큰 초과 방지.
 */
const BATCH_SIZE = 20

/**
 * text-embedding-3-small 최대 8192 토큰.
