...
> ⚠️ **정정 주석 (2026-07-13, TAX-6B-20-D)**: 아래 "OC=data 공개 테스트키로도 본문 응답이 비어
> 있음(인증 문제 아님 = API 부재 확정)"이라는 실증 결론은 이후(2026-07-08) 재조사에서 정정됐다 —
> 동일 테스트키로 `lawService.do?target=ntsCgmExpc&type=XML`을 다시 호출하니 12/12 성공했고,
> 실제 병목은 API 부재가 아니라 **회계사 운영키의 본문 조회 권한 미신청·신청 봉쇄**였다
> (TAX-6B-20-A). 이 리포트가 내린 실행 결정(해석례 실시간 경로를 목록·참고 링크로 통일)은
> 그대로 유효하다 — 이후 TAX-6B-20-A~C가 별도 오프라인 코퍼스로 본문을 확보했다(SSOT §7.2-a).
> 아래 원문은 역사 기록 보존을 위해 수정하지 않는다.
