...
1. [ ] 후보 추출 결과가 실측치와 정합 (심판례 신호 레코드 1,219건 ±5%)
2. [ ] 검수 문서에 사건번호·신호 종류·원문 발췌·검수 컬럼이 모두 포함
3. [ ] 발췌 표본 20건이 원문과 문자 단위 일치 (§6.1)
4. [ ] 검수 표에서 `확정` 행만 DB 반영되고, `해당없음`·`보류`·빈칸은 반영되지 않음 (단위 테스트)
5. [ ] 반영 스크립트 재실행 멱등
...
Report: `docs/reports/TAX-6B-33_report.md` (미작성)
