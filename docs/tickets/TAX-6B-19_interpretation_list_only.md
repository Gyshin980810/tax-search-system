...
> ⚠️ **정정 주석 (2026-07-13, TAX-6B-20-D)**: 본문 아래 "국세청 세법해석례(`ntsCgmExpc`)는 본문 조회
> API 자체가 없다"는 서술(§1.1)은 이후(2026-07-08) 재조사에서 정정됐다 — 공식 API는 존재·작동하나
> **운영키의 본문 조회 권한이 미신청·신청 봉쇄** 상태라 이 티켓이 다룬 실시간 경로에서만 못 쓰는
> 것이었다(TAX-6B-20-A). 이 티켓의 결정(해석례를 실시간에서는 목록·참고 링크로 통일)과 코드는
> **그대로 유효**하다 — 실시간 P95 보호 목적상 여전히 옳은 선택이며, 이후 TAX-6B-20-A~C가 별도
> 오프라인 코퍼스로 국세청 해석례 본문을 확보해 참고 목록 의미 검색에 반영했다(SSOT §7.2-a 참고).
